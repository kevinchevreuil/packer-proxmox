...
source "proxmox-iso" "debian-11" {
  ...
  template_description = "Kaisen Linux cloud-init template"
  ...
  iso_file       = "exo-industries-cephfs:iso/kaisenlinuxrolling2.3-amd64-NETINST.iso"
  http_directory = "./"
  boot_wait      = "10s"
# la boot command par défaut sur debian d'apres la doc pour l'autoinstallation par preseed
  boot_command   = ["<esc><wait>auto url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/kaisen-preseed.cfg<enter>"]
  unmount_iso    = true

  cloud_init              = true
  cloud_init_storage_pool = var.cloudinit_storage_pool

  vm_name  = "kaisen-template"
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "echo 'projet' | sudo -S sudo chown projet:projet /home/projet/.ssh/authorized_keys",
